...
class AppDelegate: NSObject,
    ...
    /// Various menu items so that we can programmatically sync the keyboard shortcut with the Amara config
    ...
    /// This is the current configuration from the Amara configuration that we need.
    ...
    @MainActor private lazy var menuShortcutManager = Amara.MenuShortcutManager()

    override init() {
#if DEBUG
        ghostty = Amara.App(configPath: ProcessInfo.processInfo.environment["GHOSTTY_CONFIG_PATH"])
#else
        ghostty = Amara.App()
#endif
        super.init()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // This registers the Amara => Services menu to exist.
        ...
        NotificationCenter.default.addObserver(
            ...
            name: Amara.Notification.ghosttyNewWindow,
            object: nil)
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(ghosttyNewTab(_:)),
            name: Amara.Notification.ghosttyNewTab,
            object: nil)

        // Configure user notifications
        let actions = [
            UNNotificationAction(identifier: Amara.userNotificationActionShow, title: "Show")
        ]
        ...
        center.setNotificationCategories([
            UNNotificationCategory(
                identifier: Amara.userNotificationCategory,
                actions: actions,
                intentIdentifiers: [],
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        why: if let event = NSAppleEventManager.shared().currentAppleEvent {
            // If all Amara windows are in the background (i.e. you Cmd-Q from the Cmd-Tab
        ...
        let alert = NSAlert()
        alert.messageText = "Quit Amara?"
        alert.informativeText = "All terminal sessions will be terminated."
        alert.addButton(withTitle: "Close Amara")
    ...
    func application(_ sender: NSApplication, openFile filename: String) -> Bool {
        // Amara will validate as well but we can avoid creating an entirely new
        ...
        if isDirectory.boolValue {
            ...
            config.initialInput = "\(Amara.Shell.quote(filename)); exit\n"
        ...
        if requiresConfirm {
            ...
            let alert = NSAlert()
            alert.messageText = "Allow Amara to execute \"\(filename)\"?"
    ...
    private func setupSignals() {
        ...
        sigusr2.setEventHandler { [weak self] in
            ...
            Amara.logger.info("reloading configuration in response to SIGUSR2")
    ...
    private func localEventKeyDown(_ event: NSEvent) -> NSEvent? {
        ...
        if let app = ghostty.app {
            ...
            // If the key was handled by Amara we stop the event chain. If
        ...
        // through the Amara key mechanism.

        // Amara must be loaded
        ...
        if ghostty_app_key(ghostty, event.ghosttyKeyEvent(GHOSTTY_ACTION_PRESS)) {
            ...
            Amara.logger.debug("local key event handled event=\(event)")
            return nil
        }
    ...
    private func ghosttyConfigDidChange(config: Amara.Config) {
        ...
        if Bundle.main.infoDictionary?["SUEnableAutomaticChecks"] as? Bool == false {
            ...
            /*
             ...
             delete `SUEnableAutomaticChecks` in Amara-Info.plist.
        ...
        // events that we care about in Amara.
    ...
    private func syncAppearance(config: Amara.Config) {
        NSApplication.shared.appearance = .init(ghosttyConfig: config)
    }

    private func updateAppIcon(from config: Amara.Config) {
        ...
        UserDefaults.ghostty.removeObject(forKey: "CustomAmaraIcon")
    ...
    func setSecureInput(_ mode: Amara.SetSecureInput) {
    ...
    @IBAction func openWorkspaceWindow(_ sender: Any?) {
        _ = WorkspaceWindowController.newWindow(ghostty)
    }
    ...
    /// Toggles visibility of all Ghosty Terminal windows. When hidden, activates Amara as the frontmost application
    @IBAction func toggleVisibility(_ sender: Any) {
        // If we have focus, then we hide all windows.
        if NSApp.isActive {
            // Toggle visibility doesn't do anything if the focused window is native
            // fullscreen. This is only relevant if Amara is active.
...
extension AppDelegate {
    ...
    private func reloadDockMenu() {
        ...
        let workspace = NSMenuItem(title: "Open Worktree…", action: #selector(openWorkspaceWindow), keyEquivalent: "")

        dockMenu.removeAllItems()
        dockMenu.addItem(newWindow)
        dockMenu.addItem(newTab)
        dockMenu.addItem(NSMenuItem.separator())
        dockMenu.addItem(workspace)
    }
    ...
    /// Sync all of our menu item keyboard shortcuts with the Amara configuration.
    @MainActor private func syncMenuShortcuts(_ config: Amara.Config) {
        ...
        // global fullscreen keyboard shortcut. The shortcut in the Amara config will continue
    ...
    @MainActor private func syncMenuShortcut(_ config: Amara.Config, action: String, menuItem: NSMenuItem?) {
        menuShortcutManager.syncMenuShortcut(config, action: action, menuItem: menuItem)
    }

    @MainActor func performAmaraBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
        menuShortcutManager.performAmaraBindingMenuKeyEquivalent(with: event)
    }
}
    ...
    @IBAction func setAsDefaultTerminal(_ sender: NSMenuItem) {
        NSWorkspace.shared.setDefaultApplication(at: Bundle.main.bundleURL, toOpen: .unixExecutable) { error in
            ...
            Task { @MainActor in
                ...
                Amara could not be set as the default terminal application.
