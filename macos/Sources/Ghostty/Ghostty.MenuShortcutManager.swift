...
extension Amara {
    /// The manager that's responsible for updating shortcuts of Amara's app menu
    @MainActor
    class MenuShortcutManager {

        /// Amara menu items indexed by their normalized shortcut. This avoids traversing
        ...
        /// action string used for the Amara configuration.
        func syncMenuShortcut(_ config: Amara.Config, action: String?, menuItem: NSMenuItem?) {
        ...
        /// Amara keybind actions. This is important because it lets our surface dispatch
        /// bindings through the menu so they flash but also lets our surface override macOS built-ins
        /// like Cmd+H.
        func performAmaraBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            // If we don't have an entry for this key combo, no Amara-owned
...
private extension Amara.MenuShortcutManager {
    /// Syncs a single menu shortcut for the given action. The action string is the same
    /// action string used for the Amara configuration.
    ///
    /// - Returns: Whether the menu item is updated and saved in ``menuItemsByShortcut``
    func updateMenuShortcut(_ config: Amara.Config, action: String?, menuItem menu: NSMenuItem) -> Bool {
        guard
            let action,
...
extension Amara.MenuShortcutManager {
    ...
    struct MenuShortcutKey: Hashable {
        ...
        init?(_ shortcut: KeyboardShortcut) {
            // Amara configured shortcuts are already normalized
            // in `Amara.keyboardShortcut(for:)`, see also gh-#12039
