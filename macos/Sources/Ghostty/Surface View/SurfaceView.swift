...
extension Amara {
    ...
    struct Terminal: View {
        @EnvironmentObject private var ghostty: Amara.App
            ...
            ZStack {
                ...
                if let searchState = surfaceView.searchState {
                    SurfaceSearchOverlay(
                        ...
                        onClose: {
#if canImport(AppKit)
                            Amara.moveFocus(to: surfaceView)
#endif
    ...
    struct SurfaceResizeOverlay: View {
        ...
        let overlay: Amara.Config.ResizeOverlay
        let position: Amara.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    /// A surface is terminology in Amara for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libghostty, usually from the Amara configuration.
...
private struct AmaraSurfaceViewKey: EnvironmentKey {
    static let defaultValue: Amara.SurfaceView? = nil
}

private struct AmaraLastFocusedSurfaceKey: EnvironmentKey {
    ...
    static let defaultValue: Weak<Amara.SurfaceView>? = nil
}

extension EnvironmentValues {
    var ghosttySurfaceView: Amara.SurfaceView? {
        get { self[AmaraSurfaceViewKey.self] }
        set { self[AmaraSurfaceViewKey.self] = newValue }
    }

    var ghosttyLastFocusedSurface: Weak<Amara.SurfaceView>? {
        get { self[AmaraLastFocusedSurfaceKey.self] }
        set { self[AmaraLastFocusedSurfaceKey.self] = newValue }
    }
}

extension View {
    func ghosttySurfaceView(_ surfaceView: Amara.SurfaceView?) -> some View {
    ...
    func ghosttyLastFocusedSurface(_ surfaceView: Weak<Amara.SurfaceView>?) -> some View {
...
extension FocusedValues {
    var ghosttySurfaceView: Amara.SurfaceView? {
        get { self[FocusedAmaraSurface.self] }
        set { self[FocusedAmaraSurface.self] = newValue }
    }

    struct FocusedAmaraSurface: FocusedValueKey {
        typealias Value = Amara.SurfaceView
    }

    var ghosttySurfacePwd: String? {
        get { self[FocusedAmaraSurfacePwd.self] }
        set { self[FocusedAmaraSurfacePwd.self] = newValue }
    }

    struct FocusedAmaraSurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var ghosttySurfaceCellSize: OSSize? {
        get { self[FocusedAmaraSurfaceCellSize.self] }
        set { self[FocusedAmaraSurfaceCellSize.self] = newValue }
    }

    struct FocusedAmaraSurfaceCellSize: FocusedValueKey {
