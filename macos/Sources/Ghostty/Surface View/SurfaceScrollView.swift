...
/// Wraps a Amara surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual Amara renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: Amara.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: Amara.SurfaceView) {
        ...
        // so that our primary Amara renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from Amara
        observers.append(NotificationCenter.default.addObserver(
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? Amara.Action.Scrollbar else {
            return
        }
