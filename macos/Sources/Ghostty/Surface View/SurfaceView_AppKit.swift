...
extension Amara {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
        ...
        // detected with the set_password_input_cb on the Amara state.
        ...
        private(set) var surfaceModel: Amara.Surface?
        ...
        var scrollbar: Amara.Action.Scrollbar?
        ...
        init(_ app: ghostty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            ...
            center.addObserver(
                ...
                name: Amara.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidContinueKeySequence),
                name: Amara.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidEndKeySequence),
                name: Amara.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(ghosttyDidChangeKeyTable),
                name: Amara.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
            ...
            guard let surface = surface else {
                self.error = Amara.Error.apiFailed
                return
            }
            self.surfaceModel = Amara.Surface(cSurface: surface)
        ...
        override func sizeDidChange(_ size: CGSize) {
            // Amara wants to know the actual framebuffer size... It is very important
        ...
        @objc private func ghosttyDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[Amara.Notification.KeySequenceKey] else { return }
        ...
        @objc private func ghosttyDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[Amara.Notification.KeyTableKey] as? Amara.Action.KeyTable else { return }
        ...
        @objc private func ghosttyConfigDidChange(_ notification: SwiftUI.Notification) {
            ...
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.AmaraConfigChangeKey
        ...
        @objc private func ghosttyColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.AmaraColorChangeKey
            ] as? Amara.Action.ColorChange else { return }
        ...
        override func mouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Amara.ghosttyMods(event.modifierFlags)
        ...
        override func otherMouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Amara.ghosttyMods(event.modifierFlags)
            let button = Amara.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = Amara.Input.MousePosEvent(
                x: pos.x,
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            let scrollEvent = Amara.Input.MouseScrollEvent(
                x: x,
                y: y,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify Amara first. We do this because this will let Amara handle
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsAmara = Amara.eventModifierFlags(
                mods: ghostty_surface_key_translation_mods(
                    surface,
                    Amara.ghosttyMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsAmara directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if translationModsAmara.contains(flag) {
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performAmaraBindingMenuKeyEquivalent(with: event) {
                        return true
                    }
                }
        ...
        private func keyAction(
            ...
            // character. Control characters are encoded by Amara itself.
        ...
        override func quickLook(with event: NSEvent) {
            ...
            // Amara coordinate system is top-left, convert to bottom-left for AppKit
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            content.categoryIdentifier = Amara.userNotificationCategory
            content.userInfo = [
        ...
        struct DerivedConfig {
            ...
            let backgroundBlur: Amara.Config.BackgroundBlur
            let macosWindowShadow: Bool
            let windowTitleFontFamily: String?
            let windowAppearance: NSAppearance?
            let scrollbar: Amara.Config.Scrollbar
        ...
        required convenience init(from decoder: Decoder) throws {
            // Decoding uses the global Amara app
...
extension Amara.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the Amara API. There is a race condition between getting the
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // Amara.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // Amara will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        // Amara coordinates are in top-left (0, 0) so we have to convert to
...
extension Amara.SurfaceView: NSServicesMenuRequestor {
...
extension Amara.SurfaceView: NSMenuItemValidation {
...
extension Amara.SurfaceView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        if let url = pb.string(forType: .URL) {
            ...
            content = Amara.Shell.escape(url)
...
extension Amara.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment Amara core to
    /// expose that.
    ///
