...
extension Amara {
    ...
    class Config: ObservableObject {
        // The underlying C pointer to the Amara config structure. This
        ...
        /// in the Amara configuration. For example `keybind = cmd+q=quit` in Amara
        ...
        func keyboardShortcut(for action: String) -> KeyboardShortcut? {
            ...
            return Amara.keyboardShortcut(for: trigger)
        }
#endif

        // MARK: - Configuration Values

        /// For all of the configuration values below, see the associated Amara documentation for
        ...
        var macosCustomIcon: String {
            #if os(macOS)
            let defaultValue = NSString("~/.config/ghostty/Amara.icns").expandingTildeInPath
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>?
...
#error("unsupported")
        ...
        var quickTerminalScreen: QuickTerminalScreen {
            ...
            let str = String(cString: ptr)
            return QuickTerminalScreen(fromAmaraConfig: str) ?? .main
        }
        ...
        var quickTerminalSpaceBehavior: QuickTerminalSpaceBehavior {
            ...
            let str = String(cString: ptr)
            return QuickTerminalSpaceBehavior(fromAmaraConfig: str) ?? .move
        }
        ...
        var commandPaletteEntries: [Amara.Command] {
            guard let config = self.config else { return [] }
            ...
            return buffer.map { Amara.Command(cValue: $0) }
        }
