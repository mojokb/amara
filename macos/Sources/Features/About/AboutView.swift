...
struct AboutView: View {
    ...
    private let docsURL: URL? = nil

    /// Read the commit from the bundle.
    private var build: String? { Bundle.main.infoDictionary?["CFBundleVersion"] as? String }
    private var commit: String? { Bundle.main.infoDictionary?["AmaraCommit"] as? String }
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text("Amara")
                        .bold()
                        .font(.title)
                    Text("Git worktree-based AI agent coding terminal emulator.")
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
                        .font(.caption)
                        .tint(.secondary)
                        .opacity(0.8)
                    Text("Amara is Sanskrit for \"immortal\" —\ncoding on, forever undying.")
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
                        .font(.caption)
                        .tint(.secondary)
