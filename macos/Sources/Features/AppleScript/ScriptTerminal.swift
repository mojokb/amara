...
/// AppleScript-facing wrapper around a live Amara terminal surface.
///
...
///
/// Mapping from `Amara.sdef`:
/// - `class terminal` -> this class (`@objc(AmaraAppleScriptTerminal)`).
...
final class ScriptTerminal: NSObject {
    ...
    weak var surfaceView: Amara.SurfaceView?

    init(surfaceView: Amara.SurfaceView) {
...
/// Converts four-character codes from the `split direction` enumeration in `Amara.sdef`
...
enum ScriptSplitDirection {
    ...
    var splitDirection: SplitTree<Amara.SurfaceView>.NewDirection {
        switch self {
        case .right: .right
        case .left: .left
        case .down: .down
