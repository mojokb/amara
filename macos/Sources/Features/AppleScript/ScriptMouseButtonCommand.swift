...
/// Handler for the `send mouse button` AppleScript command defined in `Amara.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="AmaraScriptMouseButtonCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(AmaraScriptMouseButtonCommand)
final class ScriptMouseButtonCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let action: Amara.Input.MouseState
...
/// Four-character codes matching the `mouse button` enumeration in `Amara.sdef`.
private enum ScriptMouseButtonValue {
    ...
    var ghosttyButton: Amara.Input.MouseButton {
        switch self {
        case .left: .left
        case .right: .right
        case .middle: .middle
