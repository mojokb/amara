...
// Application-level Cocoa scripting hooks for the Amara AppleScript dictionary.
//
...
// the runtime object behind the `application` class in `Amara.sdef`.
...
extension NSApplication {
    ...
    /// the frontmost logical Amara window.
    @objc(frontWindow)
...
extension NSApplication {
    ...
    func handleNewSurfaceConfigurationScriptCommand(_ command: NSScriptCommand) -> NSDictionary? {
        ...
        do {
            let configuration = try Amara.SurfaceConfiguration(
    ...
    func handleNewWindowScriptCommand(_ command: NSScriptCommand) -> ScriptWindow? {
        ...
        guard let appDelegate = delegate as? AppDelegate else {
            ...
            command.scriptErrorString = "Amara app delegate is unavailable."
            return nil
        }

        let baseConfig: Amara.SurfaceConfiguration?
        if let scriptRecord = command.evaluatedArguments?["configuration"] as? NSDictionary {
            do {
                baseConfig = try Amara.SurfaceConfiguration(scriptRecord: scriptRecord)
            } catch {
                command.scriptErrorNumber = errAECoercionFail
                command.scriptErrorString = error.localizedDescription
                return nil
...
@MainActor
extension NSApplication {
    /// Whether Amara should currently accept AppleScript interactions.
    ...
    fileprivate var allSurfaceViews: [Amara.SurfaceView] {
