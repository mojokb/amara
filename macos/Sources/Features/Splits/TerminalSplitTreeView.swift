...
enum TerminalSplitOperation {
    ...
    struct Resize {
        let node: SplitTree<Amara.SurfaceView>.Node
        let ratio: Double
    }

    struct Drop {
        /// The surface being dragged.
        let payload: Amara.SurfaceView

        /// The surface it was dragged onto
        let destination: Amara.SurfaceView
...
struct TerminalSplitTreeView: View {
    let tree: SplitTree<Amara.SurfaceView>
...
private struct TerminalSplitSubtreeView: View {
    @EnvironmentObject var ghostty: Amara.App
...
private struct TerminalSplitLeaf: View {
    let surfaceView: Amara.SurfaceView
    let isSplit: Bool
    let action: (TerminalSplitOperation) -> Void
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .onPreferenceChange(Amara.DraggingSurfaceKey.self) { value in
    ...
    private struct SplitDropDelegate: DropDelegate {
        ...
        let viewSize: CGSize
        let destinationSurface: Amara.SurfaceView
        let action: (TerminalSplitOperation) -> Void
        ...
        func performDrop(info: DropInfo) -> Bool {
            ...
            _ = provider.loadTransferable(type: Amara.SurfaceView.self) { [weak destinationSurface] result in
                switch result {
