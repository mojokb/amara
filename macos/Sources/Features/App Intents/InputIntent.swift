...
struct InputTextIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        guard let surface = terminal.surfaceModel else {
            throw AmaraIntentError.surfaceNotFound
        }
...
struct KeyEventIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        // Convert KeyEventMods array to Amara.Input.Mods
        let ghosttyMods = mods.reduce(Amara.Input.Mods()) { result, mod in
...
struct MouseButtonIntent: AppIntent {
    ...
    var button: Amara.Input.MouseButton
    ...
    var action: Amara.Input.MouseState
    ...
    func perform() async throws -> some IntentResult {
        ...
        let mouseEvent = Amara.Input.MouseButtonEvent(
            action: action,
            button: button,
            mods: ghosttyMods
        )
...
struct MouseScrollIntent: AppIntent {
    ...
    @Parameter(
        ...
        default: Amara.Input.Momentum.none
    )
    var momentum: Amara.Input.Momentum
...
enum KeyEventMods: String, AppEnum, CaseIterable {
    ...
    var ghosttyMod: Amara.Input.Mods {
        switch self {
        case .shift: .shift
        case .control: .ctrl
        case .option: .alt
