...
class TerminalController: BaseTerminalController, TabGroupCloseCoordinator.Controller {
    ...
    init(_ ghostty: Amara.App,
         withBaseConfig base: Amara.SurfaceConfiguration? = nil,
         withSurfaceTree tree: SplitTree<Amara.SurfaceView>? = nil,
         parent: NSWindow? = nil
    ) {
        ...
        center.addObserver(
            ...
            name: Amara.Notification.ghosttyToggleFullscreen,
            object: nil)
        center.addObserver(
            self,
        ...
        center.addObserver(
            ...
            name: Amara.Notification.ghosttyGotoTab,
            object: nil)
        center.addObserver(
            self,
    ...
    override func surfaceTreeDidChange(from: SplitTree<Amara.SurfaceView>, to: SplitTree<Amara.SurfaceView>) {
    ...
    static func newWindow(
        _ ghostty: Amara.App,
        withBaseConfig baseConfig: Amara.SurfaceConfiguration? = nil,
    ...
    /// - Parameters:
    ///   - ghostty: The Amara app instance.
    ...
    static func newWindow(
        _ ghostty: Amara.App,
        tree: SplitTree<Amara.SurfaceView>,
    ...
    @objc private func ghosttyConfigDidChange(_ notification: Notification) {
        ...
        /// ``Amara/Amara/SurfaceView/derivedConfig`` then
    ...
    private func syncAppearance(_ surfaceConfig: Amara.SurfaceView.DerivedConfig) {
    ...
    convenience init(_ ghostty: Amara.App, with undoState: UndoState) {
        ...
        if let window {
            ...
            if let focusedUUID = undoState.focusedSurface,
               let focusTarget = surfaceTree.first(where: { $0.id == focusedUUID }) {
                DispatchQueue.main.async {
                    Amara.moveFocus(to: focusTarget, from: nil)
                }
                ...
                DispatchQueue.main.async {
                    Amara.moveFocus(to: focusedSurface, from: nil)
                }
            }
        }
    }
    ...
    override func windowDidLoad() {
        ...
        // In various situations, macOS automatically tabs new windows. Amara handles
        ...
        if !window.styleMask.contains(.fullScreen) {
            ...
            // Since Amara manages tabbing manually this will never be more than one
    ...
    @IBAction func toggleAmaraFullScreen(_ sender: Any?) {
        guard let surface = focusedSurface?.surface else { return }
    ...
    override func focusedSurfaceDidChange(to: Amara.SurfaceView?) {
    ...
    private func syncAppearanceOnPropertyChange(_ surface: Amara.SurfaceView?) {
    ...
    @objc private func onMoveTab(notification: SwiftUI.Notification) {
        ...
        // Get the move action
        guard let action = notification.userInfo?[Notification.Name.AmaraMoveTabKey] as? Amara.Action.MoveTab else { return }
    ...
    @objc private func onGotoTab(notification: SwiftUI.Notification) {
        ...
        guard let tabEnumAny = notification.userInfo?[Amara.Notification.GotoTabKey] else { return }
    ...
    @objc private func onToggleFullscreen(notification: SwiftUI.Notification) {
        ...
        if let any = notification.userInfo?[Amara.Notification.FullscreenModeKey],
           let mode = any as? FullscreenMode {
            fullscreenMode = mode
        } else {
            Amara.logger.warning("no fullscreen mode specified or invalid mode, doing nothing")
            return
        }
    ...
    struct DerivedConfig {
        ...
        let macosWindowButtons: Amara.MacOSWindowButtons
        let macosTitlebarStyle: Amara.Config.MacOSTitlebarStyle
        let maximize: Bool
