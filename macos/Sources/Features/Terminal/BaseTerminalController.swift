...
/// A base class for windows that can contain Amara windows. This base class implements
/// the bare minimum functionality that every terminal window in Amara should implement.
///
...
class BaseTerminalController: NSWindowController,
    ...
    var focusedSurface: Amara.SurfaceView? {
        didSet { syncFocusToSurfaceTree() }
    }

    /// The tree of splits within this terminal window.
    @Published var surfaceTree: SplitTree<Amara.SurfaceView> = .init() {
    ...
    /// The configuration derived from the Amara config so we don't need to rely on references.
    ...
    init(_ ghostty: Amara.App,
         baseConfig base: Amara.SurfaceConfiguration? = nil,
         surfaceTree tree: SplitTree<Amara.SurfaceView>? = nil
    ) {
        ...
        self.surfaceTree = tree ?? .init(view: Amara.SurfaceView(ghostty_app, baseConfig: base))
        ...
        center.addObserver(
            ...
            name: Amara.Notification.confirmClipboard,
            object: nil)
        center.addObserver(
            self,
        ...
        center.addObserver(
            ...
            name: Amara.Notification.ghosttyCloseSurface,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidNewSplit(_:)),
            name: Amara.Notification.ghosttyNewSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidEqualizeSplits(_:)),
            name: Amara.Notification.didEqualizeSplits,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidFocusSplit(_:)),
            name: Amara.Notification.ghosttyFocusSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidToggleSplitZoom(_:)),
            name: Amara.Notification.didToggleSplitZoom,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidResizeSplit(_:)),
            name: Amara.Notification.didResizeSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(ghosttyDidPresentTerminal(_:)),
            name: Amara.Notification.ghosttyPresentTerminal,
            object: nil)
        center.addObserver(
            self,
    ...
    @discardableResult
    func newSplit(
        at oldView: Amara.SurfaceView,
        direction: SplitTree<Amara.SurfaceView>.NewDirection,
        baseConfig config: Amara.SurfaceConfiguration? = nil
    ) -> Amara.SurfaceView? {
        ...
        let newView = Amara.SurfaceView(ghostty_app, baseConfig: config)

        // Do the split
        let newTree: SplitTree<Amara.SurfaceView>
        do {
            ...
            // no big deal.
            Amara.logger.warning("failed to insert split: \(error)")
            return nil
        }
    ...
    func surfaceTreeDidChange(from: SplitTree<Amara.SurfaceView>, to: SplitTree<Amara.SurfaceView>) {
    ...
    func closeSurface(
        _ node: SplitTree<Amara.SurfaceView>.Node,
        withConfirmation: Bool = true
    ) {
        ...
        // due to SwiftUI bugs (see Amara #560). To repeat from #560, the bug is that
    ...
    private func findNextFocusTargetAfterClosing(node: SplitTree<Amara.SurfaceView>.Node) -> Amara.SurfaceView? {
    ...
    private func removeSurfaceNode(_ node: SplitTree<Amara.SurfaceView>.Node) {
        // Move focus if the closed surface was focused and we have a next target
        let nextFocus: Amara.SurfaceView? = if node.contains(
    ...
    func replaceSurfaceTree(
        _ newTree: SplitTree<Amara.SurfaceView>,
        moveFocusTo newView: Amara.SurfaceView? = nil,
        moveFocusFrom oldView: Amara.SurfaceView? = nil,
        ...
        if let newView {
            DispatchQueue.main.async {
                Amara.moveFocus(to: newView, from: oldView)
            }
        }
        ...
        undoManager.registerUndo(
            ...
            if let oldView {
                DispatchQueue.main.async {
                    Amara.moveFocus(to: oldView, from: target.focusedSurface)
                }
            }
    ...
    @objc private func ghosttyConfigDidChangeBase(_ notification: Notification) {
        ...
        guard let config = notification.userInfo?[
            Notification.Name.AmaraConfigChangeKey
        ] as? Amara.Config else { return }
    ...
    @objc private func ghosttyCommandPaletteDidToggle(_ notification: Notification) {
        guard let surfaceView = notification.object as? Amara.SurfaceView else { return }
    ...
    @objc private func ghosttyDidCloseSurface(_ notification: Notification) {
        guard let target = notification.object as? Amara.SurfaceView else { return }
    ...
    @objc private func ghosttyDidNewSplit(_ notification: Notification) {
        ...
        guard let oldView = notification.object as? Amara.SurfaceView else { return }
        guard surfaceTree.root?.node(view: oldView) != nil else { return }

        // Notification must contain our base config
        let configAny = notification.userInfo?[Amara.Notification.NewSurfaceConfigKey]
        let config = configAny as? Amara.SurfaceConfiguration

        // Determine our desired direction
        guard let directionAny = notification.userInfo?["direction"] else { return }
        guard let direction = directionAny as? ghostty_action_split_direction_e else { return }
        let splitDirection: SplitTree<Amara.SurfaceView>.NewDirection
        switch direction {
    ...
    @objc private func ghosttyDidFocusSplit(_ notification: Notification) {
        ...
        guard let directionAny = notification.userInfo?[Amara.Notification.SplitDirectionKey] else { return }
        guard let direction = directionAny as? Amara.SplitFocusDirection else { return }
        ...
        DispatchQueue.main.async {
            Amara.moveFocus(to: nextSurface, from: target)
        }
    }
    ...
    @objc private func ghosttyDidResizeSplit(_ notification: Notification) {
        ...
        guard let directionAny = notification.userInfo?[Amara.Notification.ResizeSplitDirectionKey] else { return }
        guard let direction = directionAny as? Amara.SplitResizeDirection else { return }

        guard let amountAny = notification.userInfo?[Amara.Notification.ResizeSplitAmountKey] else { return }
        guard let amount = amountAny as? UInt16 else { return }

        // Convert Amara.SplitResizeDirection to SplitTree.Spatial.Direction
        let spatialDirection: SplitTree<Amara.SurfaceView>.Spatial.Direction
        switch direction {
        ...
        do {
            ...
            Amara.logger.warning("failed to resize split: \(error)")
        }
    }
    ...
    @objc private func ghosttyDidPresentTerminal(_ notification: Notification) {
        ...
        Amara.moveFocus(to: target)
        Amara.moveFocus(to: target, delay: 0.1)
    ...
    @objc private func ghosttySurfaceDragEndedNoTarget(_ notification: Notification) {
        ...
        let newTree = SplitTree<Amara.SurfaceView>(view: target)
    ...
    private func localEventFlagsChanged(_ event: NSEvent) -> NSEvent? {
        var surfaces: [Amara.SurfaceView] = surfaceTree.map { $0 }
    ...
    func focusedSurfaceDidChange(to: Amara.SurfaceView?) {
    ...
    private func splitDidResize(node: SplitTree<Amara.SurfaceView>.Node, to newRatio: Double) {
        let resizedNode = node.resizing(to: newRatio)
        do {
            surfaceTree = try surfaceTree.replacing(node: node, with: resizedNode)
        } catch {
            Amara.logger.warning("failed to replace node during split resize: \(error)")
        }
    }

    private func splitDidDrop(
        source: Amara.SurfaceView,
        destination: Amara.SurfaceView,
        zone: TerminalSplitDropZone
    ) {
        // Map drop zone to split direction
        let direction: SplitTree<Amara.SurfaceView>.NewDirection = switch zone {
        case .top: .up
        case .bottom: .down
        case .left: .left
        case .right: .right
        ...
        if let sourceNode = surfaceTree.root?.node(view: source) {
            ...
            do {
                ...
                Amara.logger.warning("failed to insert surface during drop: \(error)")
                return
            }
        ...
        var sourceNode: SplitTree<Amara.SurfaceView>.Node?
        ...
        guard let sourceController, let sourceNode else {
            Amara.logger.warning("source surface not found in any window during drop")
            return
        }
        ...
        do {
            ...
            Amara.logger.warning("failed to insert surface during cross-window drop: \(error)")
            return
        }
    ...
    func performAction(_ action: String, on surfaceView: Amara.SurfaceView) {
    ...
    @objc private func onConfirmClipboardRequest(notification: SwiftUI.Notification) {
        ...
        guard let str = notification.userInfo?[Amara.Notification.ConfirmClipboardStrKey] as? String else { return }
        guard let state = notification.userInfo?[Amara.Notification.ConfirmClipboardStateKey] as? UnsafeMutableRawPointer? else { return }
        guard let request = notification.userInfo?[Amara.Notification.ConfirmClipboardRequestKey] as? Amara.ClipboardRequest else { return }

        // If we already have a clipboard confirmation view up, we ignore this request.
        // This shouldn't be possible...
        guard self.clipboardConfirmation == nil else {
            Amara.App.completeClipboardRequest(surface, data: "", state: state, confirmed: true)
            return
        }
    ...
    func clipboardConfirmationComplete(_ action: ClipboardConfirmationView.Action, _ request: Amara.ClipboardRequest) {
        ...
        case .osc_52_read, .paste:
            ...
            Amara.App.completeClipboardRequest(cc.surface, data: str, state: cc.state, confirmed: true)
        }
    }
    ...
    func windowDidBecomeKey(_ notification: Notification) {
        ...
        if let window, window.firstResponder == window, let focusedSurface {
            DispatchQueue.main.async {
                Amara.moveFocus(to: focusedSurface)
            }
        }
    ...
    private func splitMoveFocus(direction: Amara.SplitFocusDirection) {
        guard let surface = focusedSurface?.surface else { return }
    ...
    private struct DerivedConfig {
        let macosTitlebarProxyIcon: Amara.MacOSTitlebarProxyIcon
        let windowStepResize: Bool
        let focusFollowsMouse: Bool
        let splitPreserveZoom: Amara.Config.SplitPreserveZoom
...
extension BaseTerminalController {
    ...
    func surfaceValuesPublisher<Value>(
        valueKeyPath: KeyPath<Amara.SurfaceView, Value>,
        publisherKeyPath: KeyPath<Amara.SurfaceView, Published<Value>.Publisher>
    ) -> AnyPublisher<[Amara.SurfaceView.ID: Value], Never> {
