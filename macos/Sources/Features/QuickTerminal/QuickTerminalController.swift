...
class QuickTerminalController: BaseTerminalController {
    ...
    /// The previously running application when the terminal is shown. This is NEVER Amara.
    ...
    override func focusSurface(_ view: Amara.SurfaceView) {
        if visible {
        ...
        if surfaceTree.isEmpty,
           let ghostty_app = ghostty.app {
            if let tree = restorationState?.surfaceTree, !tree.isEmpty {
                ...
                let view = Amara.SurfaceView(ghostty_app, baseConfig: config)
        ...
        NSAnimationContext.runAnimationGroup({ context in
            ...
            DispatchQueue.main.async {
                ...
                if !NSApp.isActive {
                    ...
                    // shown on a screen that has no other Amara windows, it takes
