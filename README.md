...
<p align="center">
  ...
  <p align="center">
    A git worktree-based agent coding terminal emulator.
    <br />
    Run parallel AI agents across worktrees — fast, native, purpose-built for agentic development.
    <br />
...
**amara** is a terminal emulator purpose-built for AI-agent coding workflows, built on top of
[Ghostty](https://ghostty.org) by Mitchell Hashimoto.

Modern agentic development means running multiple AI coding agents in parallel — each on its own
`git worktree`, isolated from the others, yet all visible and manageable in one place. amara is
designed around this workflow from the ground up.

### Features

**Worktree workspace panel**
- Left sidebar lists all git worktrees (`git worktree list`), auto-refreshed as you navigate
- Clicking a worktree switches to its workspace; existing sessions are preserved in the background
- Each workspace maintains independent Claude Code and Codex PTY sessions — background sessions
  keep running even when hidden (no SIGSTOP, no eviction)

**Parallel agent tabs**
- `[claude]` and `[codex]` tabs per worktree, each backed by a dedicated persistent PTY
- Sessions start with the worktree path as the working directory
- Switching worktrees flips the visible workspace; all other agents continue working in the background

**Agent activity monitoring**
- Per-agent status dots (claude ● codex ●) on each worktree row in the sidebar
- Four states detected in real time: **idle** (gray), **active** (blue breathing), **approval needed**
  (orange pulse), **error** (red)
- Active detection combines spinner character scanning, output velocity, and shell-prompt absence
- Approval detection matches Claude Code and Codex confirmation prompts via regex

**Approval alerts**
- Background agent waiting for approval triggers an animated orange pulse on the worktree row
- macOS notification fires when the app is not in focus; tapping it jumps to the relevant worktree
- Dock badge shows total pending approval count across all worktrees
- Unacknowledged for 30 minutes → pulse demoted to a static indicator

**File browser**
- Per-worktree file tree below the worktree list, with lazy loading and a 500-item cap per directory
- Single-click inserts the relative path into the focused terminal (no execution)
- Option-click inserts the absolute path; right-click for copy / open-in-Finder options
- `.git/` always hidden; dotfiles toggled via a header button

**Editor tabs**
- Clicking a file in the browser opens it in a `nvim` / `vim` / `vi` tab (runtime detection)
- File tabs appear alongside `[claude]` and `[codex]` in a horizontally scrollable tab bar
- Closing a tab sends `:q` to let vim handle unsaved-changes warnings
- Modified indicator (`*`) via terminal title subscription (`set title` in nvimrc)

**Native macOS**
- SwiftUI + Metal renderer, built on the same high-performance Zig core as Ghostty
- Keyboard shortcuts: `Cmd+1` / `Cmd+2` for agent tabs, `Ctrl+Tab` to cycle, configurable toggle
  for the worktree panel

amara is based on Ghostty and distributed under the MIT License.
See [LICENSE](LICENSE) for details. Original Ghostty source:
